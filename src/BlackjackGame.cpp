...
void Deck::Deal(Hand& aHand, AudioManager& audio)
{
	if (!(m_Cards.empty())) {
		...
		audio.PlaySFX("media/sounds/SFX/Card_Shuffle_Full.ogg", 0.4f, 0.5f, 2.0f, 4.0f,
			      dealerPos, dealerVel);
...
void Deck::AdditionalCards(GenericPlayer& aGenericPlayer, AudioManager& audio)
{
	...
	while (!(aGenericPlayer.IsBusted()) && aGenericPlayer.IsHitting()) {
		audio.PlaySFX("media/sounds/SFX/Card_Dealt.ogg", 0.2f, 0.4f, -6.0f, 6.0f,
			      aGenericPlayer.pos, aGenericPlayer.vel);
...
void Game::Play(AudioManager& audio)
{
	...
	FMOD_VECTOR dealerPos = { 0.0f, 0.0f, 0.0f };
	FMOD_VECTOR dealerVel = { 0.0f, 0.0f, 0.0f };
	audio.PlaySFX("media/sounds/SFX/Card_Shuffle_Full.ogg", 0.4f, 0.5f, -1.4f, 1.4f, dealerPos,
		      dealerVel);
	this_thread::sleep_for(chrono::seconds(6));
	...
	for (pPlayer = m_Players.begin(); pPlayer != m_Players.end(); ++pPlayer) {
		...
		audio.PlaySFX("media/sounds/SFX/Card_Dealt_2.ogg", 0.4f, 1.0f, -2.0f, 6.0f,
			      playerPos, playerVel);
		this_thread::sleep_for(chrono::seconds(2));
		cout << *pPlayer << endl;
	}
	audio.PlaySFX("media/sounds/SFX/Card_Dealt_2.ogg", 0.4f, 1.0f, -2.0f, 6.0f, dealerPos,
		      dealerVel);
	this_thread::sleep_for(chrono::seconds(2));
	cout << m_House << endl;
	...
	if (m_House.IsBusted()) {
		...
		for (pPlayer = m_Players.begin(); pPlayer != m_Players.end(); ++pPlayer) {
			if (!(pPlayer->IsBusted())) {
				playerPos = pPlayer->getPos();
				playerVel = pPlayer->getVel();
				audio.PlaySFX("media/sounds/SFX/Clank.ogg", 0.4f, 0.8f, 1.0f, 6.0f,
					      playerPos, playerVel);
				pPlayer->Win();
				this_thread::sleep_for(chrono::seconds(1));
			}
		...
		for (pPlayer = m_Players.begin(); pPlayer != m_Players.end(); ++pPlayer) {
			if (!(pPlayer->IsBusted())) {
				if (pPlayer->GetTotal() > m_House.GetTotal()) {
					playerPos = pPlayer->getPos();
					playerVel = pPlayer->getVel();
					audio.PlaySFX("media/sounds/SFX/Clank.ogg", 0.4f, 0.8f,
						      1.0f, 6.0f, playerPos, playerVel);
					pPlayer->Win();
					this_thread::sleep_for(chrono::seconds(1));
